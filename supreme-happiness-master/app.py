...
from forms import LoginForm, SaludarForm, RegistrarForm, BuscarForm, AgregarForm
...
def agregar():
    if 'username' not in session:
        flash('Debes estar logeado para agregar compras')
        return redirect(url_for('ingresar'))
    else:
        formulario = AgregarForm()
        if formulario.validate_on_submit():
            with open('datos.csv','a') as file:
                file.write(f'{formulario.codigo.data}, {formulario.producto.data}, {formulario.cliente.data}, {formulario.cantidad.data}, {formulario.precio.data}\n')

            flash('La compra ha sido agregada')
            return redirect(url_for('agregar'))


        return render_template('agregar.html', form=formulario)
